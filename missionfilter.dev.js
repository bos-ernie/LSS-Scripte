...
(function() {
    ...
    $.getJSON('https://lssm.ledbrain.de/api/missions.php?lang='+I18n.locale).done(function(data){
        ...
        missionMarkerAdd = e => {
            ...
            var missionIsHidden = $('#mission_'+e.id).hasClass('mfhide');
            var bla = $('#mission_'+e.id).find('span[id^="mission_participant_new_"]').hasClass('hidden'); // <========= wird zu spaet erkannt .........

            if(e.caption.startsWith('[') && !missionIsHidden){
                setTimeout(() => { if(processMissionFilter(e.mtid) == "hide" || $('#mission_'+e.id).find('span[id^="mission_participant_new_"]').hasClass('hidden')) $('#mission_'+e.id).addClass('mfhide')}, 5000);
            }
        };
    });
    ...
    var initMissionFilter = () => {
        $('#mission_list_alliance > .missionSideBarEntry, #mission_list_alliance_event > .missionSideBarEntry, #mission_list_sicherheitswache > .missionSideBarEntry').each(function(){
            ...
            if(initShowHide == "hide" || $this.find('span[id^="mission_participant_new_"]').hasClass('hidden')) $this.addClass('mfhide');
        });
    }
    ...
    if(typeof alliance_id != "undefined"){
        $('#alliance_li .dropdown-menu').append(`<div class="col-sm-12">
                                                     <div class="input-group">
                                                         ...
                                                         <input type="text" maxlength="5" class="form-control" id="mfMinCredits" value=${checkCredits} style="min-width: 5em !important">
